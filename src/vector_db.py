...
def preprocess_query(query_text: str) -> str:
    """
    查询预处理：jieba分词+过滤停用词
    :param query_text: 原始查询（如“我今天没吃饭中的吃饭用方言怎么说”）
    :return: 预处理后的核心关键词（如“吃饭 方言”）
    """
    # jieba分词
...
if __name__ == "__main__":
    ...
    test_query1 = "爸爸"
    print(f"\n测试检索1：{test_query1}")
    results1 = semantic_search(test_query1, top_k=10)
    print("\n检索结果（按相似度排序）：")
    ...
    print(f"\n\n测试检索2：{test_query2}")
    results2 = semantic_search(test_query2, top_k=10)
    print("\n检索结果（按相似度排序）：")
